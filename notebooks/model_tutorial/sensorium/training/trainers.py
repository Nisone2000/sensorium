...
def standard_trainer(
    ...
    **kwargs
    ...
    def full_objective(model, dataloader, data_key, *args, **kwargs):
        KL_div = torch.zeros(1).to(device)
        loss_scale = (
        ...
        if include_KLdivergence:
            KL_div += KLDivLoss(size_average=False) #losses are summed for each minibatch
        tot_main_loss = loss_scale * criterion(
                model(args[0].to(device), data_key=data_key, **kwargs),
                args[1].to(device),) 
        return (tot_main_loss + regularizers + KL_div), (tot_main_loss, regularizers, KL_div)
